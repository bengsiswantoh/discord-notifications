  ...
  try {
    ...
    for (const type of types) {
      for (const product of data[type].mosaic[0].products) {
        ...
        const url = `${website.humbleBundle.generate}${product_url}`;

        const bundle = {
          startDate,
          endDate,
          url,
          marketing: detailed_marketing_blurb,
          shortMarketing: short_marketing_blurb,
          type: tile_stamp,
        };
        bundles.push(bundle);
      }
...
const humbleBundle = async () => {
  ...
  for (const bundle of items) {
    const { url, startDate, endDate, shortMarketing, marketing } = bundle;
